...
contract StarkyToken is BEP20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "STARKY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "STARKY::delegateBySig: invalid nonce");
        require(now <= expiry, "STARKY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "STARKY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying STARKYs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "STARKY::_writeCheckpoint: block number exceeds 32 bits");
...
// will be transferred to a governance smart contract once STARKY is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accStarkyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accStarkyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    function deposit(
        ...
        user.rewardDebt = user.amount.mul(pool.accStarkyPerShare).div(1e12);
